(function () {
  ...
  // ── Auto-detect server from where widget.js was loaded ──
  const SERVER_URL =
  ...
  const VISITOR_ID = localStorage.getItem("cb_visitor_id") || generateId("visitor");
  ...
  function injectStyles() {
    ...
    style.innerHTML = `
      ...
      .cb-msg-row { display: flex; gap: 8px; align-items: flex-end; width: 100%; min-width: 0; }
      ...
      #cb-typing { display: none; gap: 8px; align-items: flex-end; }
      .cb-typing-bubble {
        background: ${config.theme === "dark" ? "#2d2d3d" : "#ffffff"};
  ...
  function buildWidget() {
    ...
    // Avatar: logo image or emoji
    const avatarContent = config.logoUrl
      ? `<img src="${config.logoUrl}" alt="bot" />`
      : "🤖";
    ...
    wrapper.innerHTML = `
      ...
      <div id="cb-window" role="dialog" aria-label="Chat with ${config.botName}">
        <div id="cb-header">
          <div id="cb-avatar">${avatarContent}</div>
  ...
  function showUnreadBadge() {
    ...
    if (badge && !isOpen) { badge.style.display = "flex"; badge.textContent = "1"; }
  }
  ...
  // ── Avatar HTML for message bubbles ──
  function avatarHtml() {
    return config.logoUrl
      ? `<div class="cb-msg-avatar"><img src="${config.logoUrl}" alt="bot" /></div>`
  ...
  function addMessage(text, role) {
    ...
    messageHistory.push({ role: role === "user" ? "user" : "model", text });
  }
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      console.error("[Digichat] Error:", err);
    } finally {
      isTyping = false;
  ...
  function showWelcomeMessage() {
    ...
    hasGreeted = true;
    setTimeout(() => addMessage(config.welcomeMessage, "bot"), 400);
  }
    ...
    if (input) {
      input.addEventListener("keydown", (e) => {
        if (e.key === "Enter" && !e.shiftKey) {
          e.preventDefault();
          const text = input.value.trim();
          if (text) { input.value = ""; autoResizeTextarea(input); sendMessage(text); }
        }
      });
  ...
  async function fetchConfig() {
    try {
      const response = await fetch(`${SERVER_URL}/api/v1/chat/config?apiKey=${API_KEY}`);
      if (response.ok) {
        const data = await response.json();
  ...
  // ── INIT ──
  async function init() {
    await fetchConfig();   // ← loads botName, color, logoUrl, theme etc from server
    injectStyles();
    buildWidget();
    attachEvents();

    // Show unread badge after 5s to grab attention
    setTimeout(() => { if (!isOpen) showUnreadBadge(); }, 5000);
  }
