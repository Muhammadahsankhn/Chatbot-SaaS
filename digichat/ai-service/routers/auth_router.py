...
GOOGLE_REDIRECT_URI = (
    ...
    "https://digichat.digixworkspace.com/users/auth/google/callback"
)

FRONTEND_URL = os.getenv(
    "FRONTEND_URL",
    "https://digichat.digixworkspace.com"
).rstrip("/")

if FRONTEND_URL.endswith("/digichat"):
...
async def google_auth(intent: str = "login"):
    try:
        if not GOOGLE_CLIENT_ID:
            print("[Google Auth] Error: GOOGLE_CLIENT_ID is missing!")
        ...
        print(f"[Google Auth] CRITICAL ERROR: {e}")
        import traceback
        traceback.print_exc()
...
async def google_callback(code: str, response: Response, state: str = "login"):
    try:
        ...
        print(f"[Google Callback] CRITICAL ERROR: {e}")
        import traceback
        traceback.print_exc()
