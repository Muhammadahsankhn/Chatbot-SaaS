...
async def _run_url_ingestion(user_id: str, url: str, max_pages: int):
    try:
        ...
        print(f"[URL Ingest] [OK] Done for {user_id}: {result}")

    except Exception as e:
        print(f"[URL Ingest] [ERROR] Error: {e}")
...
async def _run_manual_ingestion(user_id: str, text: str, source: str):
    try:
        ...
        print(f"[Manual] [OK] Done for user {user_id}: {result}")

    except Exception as e:
        print(f"[Manual] [ERROR] Error for user {user_id}: {e}")
        import traceback
        traceback.print_exc()
...
async def _run_file_ingestion(user_id: str, filename: str, file_bytes: bytes):
    try:
        ...
        result = store_chunks(user_id, chunk_dicts, source=source)
        print(f"[Upload] [OK] Stored {len(chunks)} chunks from '{filename}' for user {user_id}: {result}")

    except ValueError as e:
        print(f"[Upload] [ERROR] Parse error for {filename}: {e}")
    except Exception as e:
        print(f"[Upload] [ERROR] Error for user {user_id}: {e}")
        import traceback
        traceback.print_exc()
