...
async def get_current_user(
    ...
    if not user_id:
        print(f"[Auth] Invalid token or decode failed. Secret used: {JWT_SECRET[:5]}...")
    ...
    try:
        ...
        print(f"[Auth] ObjectId conversion failed for {user_id}: {e}")
    ...
    if user.get("status") == "suspended":
        print(f"[Auth] User suspended: {user_id}")
